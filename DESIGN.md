...
- 2026-09-09: BI ANALYTICS layer (Dashboard rebuilt as the Analytics
  page, route unchanged). Engine: kpi.dart (net flow, savings rate,
  growth, utilization, transparent 35/30/20/15 health score),
  forecast.dart (run-rate projection with overrun and pace),
  bi_scope.dart (BiFilter plus single-batched BiSnapshot load;
  half-open scopes anchor trend and budgets to the last included
  instant). Scoped repo sums keep transfer endpoint semantics.
  Page: KPI grid, global filter bar (day/week/month/year plus
  wallet, category with parent expansion, type), forecast card,
  twin-bar trend, drill-down parent tree to transactions, wallet
  and priority breakdowns, budget-vs-actual, health score with
  input breakdown and debt facts, growth movers, scope exports
  (KPI CSV, 2000-row dataset CSV, Amiri-shaped statement PDF).
  Suite 249/249, analyzer clean, debug APK green. AI layer stays
  out by design; snapshot records are the AI-ready interface.
